...
public class d6t2 {
    ...
    public static int countAnswersInGroup(String groupAnsw) {
        ...
        List<List <Character>> groupAnswAsChars = new ArrayList<>();

        List<String> personAnswer = Arrays.asList(groupAnsw.split("\n"));

        if (personAnswer.size() == 1) {
            return personAnswer.get(0).length();
        }

        for (String ans: personAnswer) {
            ...
            groupAnswAsChars.add(chars);
        }

        for (int i = 1; i < personAnswer.size(); i++ ) {
            groupAnswAsChars.get(0).retainAll(groupAnswAsChars.get(i));
        }

        return groupAnswAsChars.get(0).size();
    }
}
